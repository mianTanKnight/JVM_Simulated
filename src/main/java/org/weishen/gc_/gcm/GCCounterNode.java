 ...
 * </ul>
 */
public interface GCCounterNode {
     ...
     */
    void addReference(GCCounterNode... references);
     ...
     */
    Set<GCCounterNode> getReference();
     ...
     */
    default void increase(GCCounterNode node, Set<GCCounterNode> visitedNodes, int GCRootCount) {
        ...
        // 递归地增加所有子节点的引用计数
        for (GCCounterNode child : node.getReference()) {
     ...
     * @param visitedNodes 访问过的节点集合，用于避免循环引用导致的无限递归。
     */
    default void decrease(GCCounterNode node, Set<GCCounterNode> visitedNodes) {
        if (!visitedNodes.contains(node)) {
            ...
            // 如果引用计数降至0或以下，可能需要执行特定的回收逻辑
            if (node.getGCRootCount() <= 0) {
                node.free();
            }
        }
    }
    ...
    /**
     * 释放当前节点的资源，模拟垃圾回收过程中的清除操作。
     * 当节点的引用计数降至零时调用此方法，表示节点不再可达。
     */
    default void free() {
        Iterator<GCCounterNode> iterator = this.getReference().iterator();
    ...
    default void freeMemory() {
        //todo 暂时只输出日志
        System.out.println(this + "已被GC回收！！！");
    }
